...
// Embedding types (embedding design §7, §8; CLAUDE.md §3 splits the public class
// by vendor). Embeddings are InvokeModel-ONLY — there is no Converse equivalent
...
# Well-known Amazon Titan text-embedding model ids (embedding design §8).
...
# Well-known Cohere Embed model ids (embedding design §8).
...
# Cohere's REQUIRED `input_type` (embedding design §6). Getting this wrong
# silently degrades retrieval — there is no error, just worse results. Embed your
# corpus with `SEARCH_DOCUMENT` and your queries with `SEARCH_QUERY`.
...
# Cohere's `truncate` behaviour for over-long inputs (embedding design §7).
...
# Titan-specific embedding configuration (CLAUDE.md §3).
public type TitanEmbeddingConfig record {|
    ...
    boolean normalize?;
    # Escape hatch, mirroring the model provider's §9.3 passthrough.
...
# Cohere-specific embedding configuration (CLAUDE.md §3). `inputType` lives ONLY
# here, which is why the model provider's "inputType set on Titan → error" check
# is gone: with the vendor split it is unrepresentable.
public type CohereEmbeddingConfig record {|
    # REQUIRED on the wire (embedding design §6). Defaults to `SEARCH_DOCUMENT` —
    # the ingest path is the higher-volume one. Construct a second provider with
...
public type EmbeddingParams record {|
    ...
    # Titan only.
    boolean normalize?;
    # Cohere only — required on the wire (§6).
    CohereInputType inputType?;
    # Cohere only.
    Truncate truncate?;
    # Escape-hatch passthrough, mirroring §9.3.
    json additionalModelRequestFields?;
|};
