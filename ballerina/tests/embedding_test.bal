...
function testTitanRejectsACohereModelId() {
    TitanEmbeddingProvider|ai:Error provider = new (TEST_CREDS, "cohere.embed-english-v3", REGION);
...
function testCohereRejectsATitanModelId() {
    CohereEmbeddingProvider|ai:Error provider = new (TEST_CREDS, "amazon.titan-embed-text-v2:0", REGION);
    test:assertTrue(provider is ai:Error);
    if provider is ai:Error {
...
function testTitanRejectsAnUnsupportedDimensions() {
    TitanEmbeddingProvider|ai:Error provider = new (TEST_CREDS, TITAN_EMBED_TEXT_V2, "us-east-1", dimensions = 999);
...
function testTitanV1RejectsDimensionsEntirely() {
    TitanEmbeddingProvider|ai:Error provider = new (TEST_CREDS, TITAN_EMBED_TEXT_V1, "us-east-1", dimensions = 512);
...
function testTitanAcceptsItsThreeSupportedWidths() returns error? {
    foreach int d in [256, 512, 1024] {
        TitanEmbeddingProvider _ = check new (TEST_CREDS, TITAN_EMBED_TEXT_V2, "us-east-1", dimensions = d);
    }
}

@test:Config {}
function testCohereV3RejectsDimensionsBecauseTheModelHasNone() {
    CohereEmbeddingProvider|ai:Error provider = new (TEST_CREDS, COHERE_EMBED_ENGLISH_V3, "us-east-1",
            dimensions = 256);
...
function testCohereV4RejectsAnUnsupportedDimensions() {
    CohereEmbeddingProvider|ai:Error provider = new (TEST_CREDS, COHERE_EMBED_V4, "us-east-1", dimensions = 999);
    test:assertTrue(provider is ai:Error);
}
