...
public isolated distinct client class BedrockRuntimeAmazonModelProvider {
    *ai:ModelProvider;
    ...
    # + model - A Amazon model id. Any id the endpoint serves may be passed as a `string`
    ...
    public isolated function init(
            ...
            @display {label: "API Shape"} CoreRuntimeApi api = CONVERSE,
            @display {label: "Endpoint Configuration"} aws:EndpointConfig? endpoint = (),
            @display {label: "Maximum Tokens"} int? maxTokens = DEFAULT_MAX_TOKEN_COUNT,
            @display {label: "Temperature"} decimal? temperature = (),
            @display {label: "Configuration"} *AmazonRuntimeConfig config)
            returns ai:Error? {
        ...
        check validateParamsForRoute("BedrockRuntimeAmazonModelProvider", route.shape, converter, resolvedParams);
    ...
    isolated remote function chat(ai:ChatMessage[]|ai:ChatUserMessage messages,
        ...
        => runChat("Amazon", self.shape, self.wireModelId, self.converter, self.transport,
